...
ostream& operator<<(ostream& os, CSommet &SOMParam) { //Ca permet d'utiliser direct cout<< pour afficher un sommet
	...
	if (vARCArrivant.size() > 0) {
		os << "Point(s) venant vers " << SOMParam.SOMObtenirNumero() << " sont : " << vARCArrivant[uiBoucle].ARCObtenirSommet()->SOMObtenirNumero();
	...
	if (vARCPartant.size() > 0) {
		os << "Point(s) partant de " << SOMParam.SOMObtenirNumero() << " sont : " << vARCPartant[uiBoucle].ARCObtenirSommet()->SOMObtenirNumero();
